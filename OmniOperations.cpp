...
void OmniOperations::init(cv::Mat& inputImg) {
	...
	dBlurredImg = new cv::cuda::GpuMat(resHeight, resWidth, CV_MAKETYPE(CV_8U, numChannels));
...
cv::Mat* OmniOperations::unwrap(cv::Mat& inputImg, bool isSmoothOnGPU) { 
	...
	if(!inputImg.empty()) { 
		...
		if(dUnwrappedImg != NULL) {
			...
			//*****************************************************************************
			cv::cuda::GpuMat dImg(resHeight,
								 resWidth,
								 type,
								 (void*)dUnwrappedImg);
			if(isSmoothOnGPU) {
        gaussianFilterGpu = cv::cuda::createGaussianFilter(dImg.type(), 
                              dBlurredImg->type(),
